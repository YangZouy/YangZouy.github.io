...
(function () {
  ...
  // - 生产环境 → 自有服务器 rag.zyydgrbk.top（Nginx 反代，FastAPI root_path=/api）
  //   RAG_SEARCH_ENDPOINT 仍可作为 HTML 注入覆盖项，便于临时切换 / 回滚到 Vercel
  var ENDPOINT;
  if (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1") {
    ENDPOINT = "http://localhost:8000/search";
  } else {
    ENDPOINT = window.RAG_SEARCH_ENDPOINT || "https://rag.zyydgrbk.top/api/search";
  }
  ...
  var SHOW_TRACE = window.RAG_SHOW_TRACE === true;

  var STAGE_TEXT = {
    routing: "正在理解你的问题…",
    rewriting: "正在补全追问上下文…",
    decomposing: "正在拆分复杂问题…",
    retrieving: "正在检索博客资料…",
    remedy: "正在扩大检索范围…",
    generating: "正在整理答案…",
  };
  ...
  function renderTrace(trace) {
    if (!SHOW_TRACE || !trace) return "";
    var labels = { answer: "回答", partial_answer: "部分回答", refuse: "拒答", chat: "闲聊", out_of_scope: "超出范围", error: "错误" };
    var evidence = (trace.evidence_statuses || []).join(" → ") || "无";
    return '<details class="rag-trace"><summary>运行轨迹</summary><dl>' +
      '<dt>问题类型</dt><dd>' + escapeHtml(trace.question_type || "simple") + '</dd>' +
      '<dt>子问题数</dt><dd>' + escapeHtml(String(trace.sub_query_count || 0)) + '</dd>' +
      '<dt>检索轮数</dt><dd>' + escapeHtml(String(trace.retrieval_rounds || 0)) + '</dd>' +
      '<dt>证据状态</dt><dd>' + escapeHtml(evidence) + '</dd>' +
      '<dt>补救动作</dt><dd>' + escapeHtml(trace.remedy_action || "无") + '</dd>' +
      '<dt>最终决策</dt><dd>' + escapeHtml(labels[trace.final_decision] || trace.final_decision || "") + '</dd>' +
      '<dt>总耗时</dt><dd>' + escapeHtml(String(trace.total_latency_ms || 0)) + ' ms</dd>' +
      '</dl></details>';
  }
  ...
  function renderWelcome() {
    ...
    wrap.innerHTML =
      ...
      '<button class="rag-suggest" data-q="博客里有哪些与 AI 相关的文章？">博客里有哪些与 AI 相关的文章？</button>' +
  ...
  /**
   * response.body 是 ReadableStream（流式响应体）
   * getReader() 拿到 reader 才能一段段读。
   * 如果浏览器不支持（极老环境），直接 reject，走到后面的 JSON 降级。
   */
  function consumeSSE(response, onEvent) {
    if (!response.body || !response.body.getReader) {
      return Promise.reject(new Error("不支持流式响应"));
    }
    // 从流里读chunk
    var reader = response.body.getReader();
    // 把二进制Unit8Array解码成字符串，
    var decoder = new TextDecoder("utf-8");
    // 缓冲区：网络可能将一条SSE消息拆成2个chunk，也可能将两条消息放在1个chunk，
    // 必须先缓冲，按边界切分
    var buffer = "";
    ...
    function read() {
      return reader.read().then(function (result) {
        ...
        // 流结束，buffer还有残留，最后一条消息可能没有空行结尾，补解析一次
        if (result.done) {
  ...
  function askAI(query, messagesEl, send) {
    ...
    function renderLegacy(data) {
      var html = responseHtml(data) + renderTrace(data.trace);
    ...
    // 使用fetch+readableStream而不是eventSource
    // 选型决策：1、后者只支持GET，无法POST body
    // 2、后者不能自定义请求头 3、fetch能拿到AbortController做超时中断
      ...
      .then(function (res) {
        ...
        return consumeSSE(res, function (event, data) {
          ...
          if (event === "sources") {
            ...
            setCitations(data.citations || []);
            finishAnswer();
            if (!trace) trace = data.trace || null;
            if (SHOW_TRACE && trace) botNode.insertAdjacentHTML("beforeend", renderTrace(trace));
            var html = responseHtml(data) + renderTrace(trace);
            cache[cacheKey] = { html: html, t: Date.now() };
          } else if (event === "stage") {
            if (loadingSpan && loadingSpan.parentNode && STAGE_TEXT[data.stage]) {
              loadingSpan.textContent = STAGE_TEXT[data.stage]
            }
          }
          scrollMessages(messagesEl);
        });
